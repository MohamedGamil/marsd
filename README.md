...
<!-- <p align="center">
  <a href="https://marsd.app"><img src="https://img.shields.io/badge/Web_App-marsd.app-blue?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Web App"></a>&nbsp;
  <a href="https://tech.marsd.app"><img src="https://img.shields.io/badge/Tech_Variant-tech.marsd.app-0891b2?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Tech Variant"></a>&nbsp;
  <a href="https://finance.marsd.app"><img src="https://img.shields.io/badge/Finance_Variant-finance.marsd.app-059669?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Finance Variant"></a>
</p> -->

<!-- <p align="center">
  <a href="https://marsd.app/api/download?platform=windows-exe"><img src="https://img.shields.io/badge/Download-Windows_(.exe)-0078D4?style=for-the-badge&logo=windows&logoColor=white" alt="Download Windows"></a>&nbsp;
  <a href="https://marsd.app/api/download?platform=macos-arm64"><img src="https://img.shields.io/badge/Download-macOS_Apple_Silicon-000000?style=for-the-badge&logo=apple&logoColor=white" alt="Download macOS ARM"></a>&nbsp;
  <a href="https://marsd.app/api/download?platform=macos-x64"><img src="https://img.shields.io/badge/Download-macOS_Intel-555555?style=for-the-badge&logo=apple&logoColor=white" alt="Download macOS Intel"></a>&nbsp;
  <a href="https://marsd.app/api/download?platform=linux-appimage"><img src="https://img.shields.io/badge/Download-Linux_(.AppImage)-FCC624?style=for-the-badge&logo=linux&logoColor=black" alt="Download Linux"></a>
</p> -->
...
- **Cloud fallback** — when a local API handler fails or is missing, requests transparently fall through to the cloud deployment (marsd.app) with origin headers stripped
...
The desktop app checks for new versions by polling `marsd.app/api/version` — once at startup (after a 5-second delay) and then every 6 hours. When a newer version is detected (semver comparison), a non-intrusive update badge appears with a direct link to the GitHub Release page.
...
All processing happens server-side on Vercel Edge Functions. API keys are stored in Vercel environment variables. News feeds are proxied through domain-allowlisted endpoints. AI summaries use Groq or OpenRouter. This is the default for `marsd.app` — convenient but cloud-dependent.
...
| --------------------- | ---------------------------------------------------- | ----------------------------------------------- | ------------------------------------------------ | ----------------------------------------------------- |
| **Domain**            | marsd.app                                     | tech.marsd.app                           | finance.marsd.app                         | happy.marsd.app                                |
...
**Single-deployment consolidation** — all four variants serve from a single Vercel deployment. The variant is determined at runtime by hostname detection (`tech.marsd.app` → tech, `finance.marsd.app` → finance, `happy.marsd.app` → happy, default → full). This replaced the original multi-deployment approach, providing 4× higher CDN cache hit rates (shared static assets), a single CI pipeline with zero cross-deployment configuration drift, and instant switching without DNS lookups. Social bot OG responses generate variant-specific preview cards based on the requesting hostname.
...
Every data endpoint is accessible programmatically via `api.marsd.app`. The API uses the same edge functions that power the dashboard, with the same caching and rate limiting:

```bash
# Fetch market quotes
curl -s 'https://api.marsd.app/api/market/v1/list-market-quotes?symbols=AAPL,MSFT,GOOGL'

# Get airport delays
curl -s 'https://api.marsd.app/api/aviation/v1/list-airport-delays'

# Fetch climate anomalies
curl -s 'https://api.marsd.app/api/climate/v1/list-climate-anomalies'

# Get earthquake data
curl -s 'https://api.marsd.app/api/seismology/v1/list-earthquakes'

# Company enrichment (GitHub, SEC filings, HN mentions)
curl -s 'https://api.marsd.app/api/enrichment/company?domain=stripe.com'

# Company signal discovery (funding, hiring, exec changes)
curl -s 'https://api.marsd.app/api/enrichment/signals?company=Stripe&domain=stripe.com'
```
...
> **Note**: Use `api.marsd.app`, not `marsd.app` — the main domain requires browser origin headers and returns 403 for programmatic access.
...
| Layer                          | Mechanism                                                                                                                                                                                                                                          |
| ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **CORS origin allowlist**      | Only `marsd.app`, `tech.marsd.app`, `finance.marsd.app`, and `localhost:*` can call API endpoints. All others receive 403. Implemented in `api/_cors.js`.                                                                     |
...
```bash
# Development
npm run dev          # Full variant (marsd.app)
npm run dev:tech     # Tech variant (tech.marsd.app)
npm run dev:finance  # Finance variant (finance.marsd.app)
npm run dev:happy    # Happy variant (happy.marsd.app)
...
- [x] Programmatic API access via `api.marsd.app` (22 service domains, REST + query param support)
...
<p align="center">
  <a href="https://marsd.app">marsd.app</a> &nbsp;·&nbsp;
  <a href="https://tech.marsd.app">tech.marsd.app</a> &nbsp;·&nbsp;
  <a href="https://finance.marsd.app">finance.marsd.app</a>
</p>
