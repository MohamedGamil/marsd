...
const KEYRING_SERVICE: &str = "marsd";
...
fn open_live_channels_window(app: &AppHandle, base_url: Option<String>) -> Result<(), String> {
    ...
    .title("Channel management - Marsd")
...
fn build_app_menu(handle: &AppHandle) -> tauri::Result<Menu<tauri::Wry>> {
    ...
    let about_item =
        PredefinedMenuItem::about(handle, Some("About Marsd"), Some(about_metadata))?;
...
fn handle_menu_event(app: &AppHandle, event: tauri::menu::MenuEvent) {
    match event.id().as_ref() {
        ...
        MENU_HELP_GITHUB_ID => {
            let _ = open_in_shell("https://github.com/MohamedGamil/marsd");
        }
        #[cfg(feature = "devtools")]
        MENU_HELP_DEVTOOLS_ID => {
...
fn main() {
    ...
    tauri::Builder::default()
        ...
        .expect("error while running marsd tauri application")
        .run(|app, event| {
            match &event {
