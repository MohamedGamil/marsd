...
import { deduplicateHeadlines } from '../server/marsd/news/v1/dedup.mjs';
...
describe('getHumanitarianSummary handler', () => {
  const src = readSrc('server/marsd/conflict/v1/get-humanitarian-summary.ts');
...
describe('humanitarian_summary.proto', () => {
  const proto = readSrc('proto/marsd/conflict/v1/humanitarian_summary.proto');
...
describe('LLM prompt political context (LOW-1)', () => {
  const src = readSrc('server/marsd/news/v1/_shared.ts');
...
describe('getCacheKey determinism', () => {
  ...
  const sharedSrc = readSrc('server/marsd/news/v1/_shared.ts');
...
describe('getVesselSnapshot caching (HIGH-1)', () => {
  const src = readSrc('server/marsd/maritime/v1/get-vessel-snapshot.ts');
