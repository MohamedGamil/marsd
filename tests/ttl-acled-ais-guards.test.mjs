...
describe('cache TTL alignment with upstream refresh rates', () => {
  it('climate anomalies uses 3h TTL (ERA5 has 2-7 day lag)', () => {
    const src = readSrc('server/marsd/climate/v1/list-climate-anomalies.ts');
  ...
  it('fire detections uses 1h TTL (FIRMS NRT updates every ~3h)', () => {
    const src = readSrc('server/marsd/wildfire/v1/list-fire-detections.ts');
...
describe('ACLED consumers use shared cache layer', () => {
  it('conflict handler imports fetchAcledCached', () => {
    const src = readSrc('server/marsd/conflict/v1/list-acled-events.ts');
  ...
  it('unrest handler imports fetchAcledCached', () => {
    const src = readSrc('server/marsd/unrest/v1/list-unrest-events.ts');
    assert.match(src, /fetchAcledCached/,
      'Unrest handler should use shared ACLED fetch');
  });

  it('risk scores handler imports fetchAcledCached', () => {
    const src = readSrc('server/marsd/intelligence/v1/get-risk-scores.ts');
    assert.match(src, /fetchAcledCached/,
      'Risk scores handler should use shared ACLED fetch');
  });

  it('no handler has its own ACLED_API_URL constant', () => {
    const conflict = readSrc('server/marsd/conflict/v1/list-acled-events.ts');
    const unrest = readSrc('server/marsd/unrest/v1/list-unrest-events.ts');
    const riskScores = readSrc('server/marsd/intelligence/v1/get-risk-scores.ts');
