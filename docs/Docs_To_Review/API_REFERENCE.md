...
> Comprehensive reference for all Vercel Edge Function endpoints powering the Marsd intelligence dashboard.

**Base URL**: All endpoints are relative to `/api/` (e.g., `https://marsd.app/api/earthquakes`).
...
Marsd exposes **60+ serverless endpoints** deployed as **Vercel Edge Functions** (unless noted otherwise). Every endpoint:
...
| # | Pattern | Matches |
|---|---------|---------|
| 1 | `marsd\.app$` | `https://marsd.app` |
| 2 | `\.marsd\.app$` | `https://*.marsd.app` |
...
Marsd uses a **multi-tier caching strategy** to minimize upstream API calls, reduce latency, and stay within third-party rate limits.
