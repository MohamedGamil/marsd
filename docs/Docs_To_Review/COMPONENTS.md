...
Marsd's UI is built entirely with **vanilla TypeScript** — no React, Vue, or
...
`localStorage['marsd-panel-spans']` as a JSON map of `{ [id]: height }`.
...
Marsd ships two independent map renderers and an adapter that picks the
...
Recent selections stored in `localStorage['marsd_recent_searches']`
(most recent 10).
