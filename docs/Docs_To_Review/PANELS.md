...
> **Marsd** — Config-driven panel architecture powering three site variants.
>
...
4. [Full Variant Panels](#4-full-variant-panels-marsdio)
5. [Tech Variant Panels](#5-tech-variant-panels-techmarsdio)
6. [Finance Variant Panels](#6-finance-variant-panels-financemarsdio)
...
Marsd uses a **config-driven panel system** where every dashboard tile — from live news feeds to AI insights to market data — is declared as a `PanelConfig` entry inside a variant-specific configuration object. The system is designed around three principles:
...
|---------|--------|-------|-------------|
| `full` | marsd.io | Geopolitical intelligence, OSINT, defense | 37 |
| `tech` | tech.marsd.io | Technology, AI/ML, startups, VC | 34 |
| `finance` | finance.marsd.io | Markets, trading, macro, commodities | 29 |
...
export const STORAGE_KEYS = {
  panels:        'marsd-panels',          // Panel visibility toggles
  monitors:      'marsd-monitors',         // Monitor keyword configs
  mapLayers:     'marsd-layers',           // Map layer toggles
  disabledFeeds: 'marsd-disabled-feeds',   // Per-source feed disabling
} as const;
```
...
|-----|---------|------------|
| `marsd-panel-spans` | Panel height/span sizes (1–4) | `Panel.ts` |
| `panel-order` | Drag-and-drop panel ordering | `App.ts` |
| `marsd-variant` | Last-active variant (triggers reset on change) | `App.ts` |
| `marsd-panel-order-v1.9` | Migration flag for v1.9 panel layout | `App.ts` |
...
Span values are persisted per-panel in the `marsd-panel-spans` localStorage key as a JSON object `{ [panelId]: spanNumber }`.
...
## 4. Full Variant Panels (marsd.io)
...
## 5. Tech Variant Panels (tech.marsd.io)
...
## 6. Finance Variant Panels (finance.marsd.io)
...
|---------|-----------------|--------|----------------|:--------------:|
| Panel visibility | `marsd-panels` | `Record<string, PanelConfig>` JSON | `DEFAULT_PANELS` | ✅ |
| Panel ordering | `panel-order` | `string[]` JSON | Config declaration order | ✅ |
| Panel sizes/spans | `marsd-panel-spans` | `Record<string, number>` JSON | All span-1 | ✅ |
| Map layer toggles | `marsd-layers` | `MapLayers` JSON | `DEFAULT_MAP_LAYERS` | ✅ |
| Monitor keywords | `marsd-monitors` | `Monitor[]` JSON | `[]` | ✅ |
| Disabled sources | `marsd-disabled-feeds` | `string[]` JSON | `[]` | ✅ |
| Active variant | `marsd-variant` | Plain string | `SITE_VARIANT` | ✅ |
| Banner dismissal | `banner-dismissed` (sessionStorage) | Timestamp string | — | Session only |

### 9.2 Variant Change Reset

When the stored variant (`marsd-variant`) differs from the current `SITE_VARIANT`, the App constructor performs a full reset:

```typescript
if (storedVariant !== currentVariant) {
  localStorage.setItem('marsd-variant', currentVariant);
...
This ensures users switching between variant domains (e.g. from marsd.io to tech.marsd.io) get a clean default experience for the new variant.
...
Step-by-step guide to adding a new panel to Marsd.
...
sequenceDiagram
    ...
    App->>localStorage: saveToStorage('marsd-panels', panelSettings)
    ...
    App->>localStorage: loadFromStorage('marsd-panels')
