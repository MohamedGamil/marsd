...
Marsd is an AI-powered real-time global intelligence dashboard built with **vanilla TypeScript** — no framework, no reactive stores. All state is managed manually through class properties, `localStorage`, `IndexedDB`, and URL query parameters.
...
```typescript
const PANEL_SPANS_KEY = 'marsd-panel-spans';
...
Stored in `localStorage` under `marsd-panels` as `Record<string, PanelConfig>`. Includes per-panel `enabled` state, `name`, and `priority`. Controlled by the variant config and user overrides.
...
| Key | Type | Purpose |
|-----|------|---------|
| `marsd-panels` | `Record<string, PanelConfig>` | Per-panel enabled/name/priority |
| `marsd-monitors` | `Monitor[]` | Color/label configs for monitors |
| `marsd-layers` | `MapLayers` | Enabled/disabled map layer toggles |
| `marsd-disabled-feeds` | `string[]` | User-disabled news feed sources |
...
sequenceDiagram
    ...
    setTheme->>localStorage: setItem('marsd-theme', 'light')
...
- **Toggles key:** `marsd-runtime-feature-toggles` in `localStorage`
...
Complete reference of every `localStorage` key used by Marsd:

| Key | Purpose | Format | Source |
|-----|---------|--------|--------|
| `marsd-variant` | Active variant override | `'full' \| 'tech' \| 'finance'` | `src/config/variant.ts`, `App.ts` |
| `marsd-theme` | Theme preference | `'dark' \| 'light'` | `src/utils/theme-manager.ts` |
| `panel-order` | Panel arrangement order | `string[]` (JSON) | `App.ts` |
| `marsd-panel-spans` | Panel grid sizes | `Record<string, number>` (JSON) | `src/components/Panel.ts` |
| `marsd-panels` | Panel enabled/config state | `Record<string, PanelConfig>` (JSON) | `STORAGE_KEYS.panels` |
| `marsd-monitors` | Monitor color/label configs | `Monitor[]` (JSON) | `STORAGE_KEYS.monitors` |
| `marsd-layers` | Map layer toggles | `MapLayers` (JSON) | `STORAGE_KEYS.mapLayers` |
| `marsd-disabled-feeds` | User-disabled news sources | `string[]` (JSON) | `STORAGE_KEYS.disabledFeeds` |
| `marsd-runtime-feature-toggles` | Desktop feature toggles | `Record<RuntimeFeatureId, boolean>` (JSON) | `src/services/runtime-config.ts` |
| `marsd-persistent-cache:{key}` | Persistent data cache entries | `CacheEnvelope<T>` (JSON) | `src/services/persistent-cache.ts` |
| `wm-update-dismissed-{version}` | Dismissed update notifications | `'1'` | `App.ts` |
| `marsd-panel-order-v1.9` | Panel order migration flag | `'done'` | `App.ts` (one-time migration) |
| `marsd-tech-insights-top-v1` | Tech variant migration flag | `'done'` | `App.ts` (one-time migration) |
