...
const BROWSER_ORIGIN_PATTERNS = [
  /^https:\/\/(.*\.)?marsd\.app$/,
  /^https:\/\/marsd-[a-z0-9-]+-elie-[a-z0-9]+\.vercel\.app$/,
...
export function validateApiKey(req) {
  const key = req.headers.get('X-Marsd-Key');
  ...
  if (isDesktopOrigin(origin)) {
    ...
    const validKeys = (process.env.MARSD_VALID_KEYS || '').split(',').filter(Boolean);
    if (!validKeys.includes(key)) return { valid: false, required: true, error: 'Invalid API key' };
    return { valid: true, required: true };
  }

  // Trusted browser origin (marsd.app, Vercel previews, localhost dev) — no key needed
