...
export class DeductionPanel extends Panel {
    ...
    private async handleSubmit(e: Event) {
        ...
        try {
            ...
            const resp = await fetch(`/api/intelligence/v1/deduct-situation?lang=${encodeURIComponent(lang)}`, {
                method: 'POST',
            ...
            const data = await resp.json() as { analysis?: string; provider?: string; model?: string };
            this.resultContainer.className = 'deduction-result';
            if (data.analysis) {
                const parsed = await marked.parse(data.analysis);
                if (!this.element?.isConnected) return;
                ...
                const meta = h('div', { style: 'margin-top: 12px; font-size: 0.75em; color: #888;' },
                    `Generated by ${data.provider || 'AI'}${data.model ? ` (${data.model})` : ''}`
                );
